...
class ESN():
  ...
  def __init__(self, rho =0.9, Nr=100, Nu=1, r_density =0.2, i_density =0.1, Ny=1):
    ...
    # maschera dropout
    self.D = np.zeros(self.W.shape)      ; self.D[:,:] = self.W.todense()[:,:] != 0 
    self.D_in = np.zeros(self.W_in.shape); self.D_in[:,:] = self.W_in.todense()[:,:] != 0 
  ...
  def build_input_matrix(self):
    ...
    return s.csr_array(w_in)

  def compute_state(self, u):
    u = np.vstack( (u,1) )
    z = self.W_in.dot( u ) + self.W.dot( self.x )
    output = np.tanh( z )
    self.x = output
...
def eval_shallow_one(train_data, test_data, tresh=0.0001 , max_epochs = 1000 , step=1e-05 , train_fun=train_input , r_density=0.2 , i_density=0.1 , Nr=100, Nu=1 , mesure_interval=50, rho=0.9):
  ...
  # rhos = np.array( max(np.abs(np.linalg.eigvals(esn.W.todense()))) )
  ...
  rhos.append( max(np.abs(np.linalg.eigvals(esn.W.todense()))) ) 
  ...
  print("norma pesi in alla fine: ", np.linalg.norm(esn.W_in.todense()))
  print("norma pesi rec alla fine: ", np.linalg.norm(esn.W.todense()))
  print("raggio spettrale matrice ricorrente alla fine: ",max(np.abs(np.linalg.eigvals(esn.W.todense()))) ,'\n' )
